...
def main():
    ...
    for s in ml_data['sites']:
        ...
        dist = calc_gcd(last_latitude, last_longitude, next_latitude, next_longitude)
        time = compute_time(dist)

        collection_time = composition_times[composition]

        print(
            f"leg =  {stop} , distance traveled = {dist}, time to travel = {time} hrs, time to sample = {collection_time} hrs")
        print()
    ...
    stop -= 1
    print(
        f" Number of legs: {stop}, total time elapsed = {total_travel_time} hrs, over a total distance of {total_travel_distance} km")
